...
def _start():
    ...
    with open(join(resources.screen_data_path, 'menu.txt'), 'r', encoding='utf-8') as f:
        input = f.readline()
...
def _update():
    ...
    if key_state_tracker.get_key_state('s', key_state_tracker.JUST_PRESSED) or key_state_tracker.get_key_state('down', key_state_tracker.JUST_PRESSED): index = clip(index + 1, 0, max_index)
    elif key_state_tracker.get_key_state('w', key_state_tracker.JUST_PRESSED) or key_state_tracker.get_key_state('up', key_state_tracker.JUST_PRESSED): index = clip(index - 1, 0, max_index)
    elif key_state_tracker.get_key_state('enter', key_state_tracker.JUST_PRESSED): 
